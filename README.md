...
- A builder ([builder/podman](builder/podman))
- A provisioner ([provisioner/podman](provisioner/podman))
- A post-processor ([post-processor/podman](provisioner/podman))
- A data source ([datasource/podman](datasource/podman))
- Docs ([docs](docs))
...
This podman configures a [manually triggered plugin test action](/.github/workflows/test-plugin-example.yml).
...
To include plugin docs on Packer.io a global pre-hook has been added to the main podman .goreleaser.yml file, that if uncommented will generate and include a docs.zip file as part of the plugin release.
...
```json
{
   "title": "podman",
   "path": "podman",
   "repo": "hashicorp/packer-plugin-podman",
   "version": "latest",
   "sourceBranch": "main"
 }
```
